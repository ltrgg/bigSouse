...
/*-API-*/
const multiparty = require("formidable"),
...
function handleMultiparty(req, res, temp) {
    return new Promise((resolve, reject) => {
        ...
        let form = new multiparty.IncomingForm(options);
        // multiparty解析
        form.parse(req, function (err, fields, files,go='返回') {
            if (err) {
                res.send({
                    code: 1,
                    reason: err
...
app.post('/single3', async (req, res) => {
    let {files,fields} = await handleMultiparty(req, res, true);
    ...
    // path = `${path}/${filename}`;
       console.log(path,3456);
    fs.access(path, async err => {
        // 存在的则不再进行任何的处理
        if (!err) {
            res.send({
                code: 0,
                path: path.replace(`${__dirname}/${filename}`, `http://127.0.0.1:${PORT}`)
            });
            return;
        }
